  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => ({ opacity: pressed ? 0.9 : 1 })}>
      <LinearGradient
        ...
        style={{ width: '100%', minHeight: 48, paddingVertical: 13, borderRadius: 12, alignItems: 'center' }}>
        <Text style={{ color: '#fff', fontSize: 15, fontWeight: '700' }}>{label}</Text>
      </LinearGradient>
    </Pressable>
  );
  ...
  return (
    <View>
      <Field
        ...
        onBlur={() => {
          // Tocar fuera del campo debe cerrar la lista para que no quede
          // flotando sobre botones u otros controles.
          setOpen(false);
        }}
      />
      {open && items.length > 0 ? (
